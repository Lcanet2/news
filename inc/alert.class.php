...
class PluginNewsAlert extends CommonDBTM
{
    ...
    public static function findAllToNotify($params = [])
    {
        ...
        if (!$p["show_only_login_alerts"]) {
            $entity_sql = getEntitiesRestrictRequest(
                "AND",
                $table,
                "",
                $p["entities_id"],
    ...
    private function validateFormInput(array $input, $isUpdate = false)
    {
        ...
        if (!$isUpdate || $messageProvided) {
            $messageToCheck = $messageProvided
                ? $input["message"]
                : $this->fields["message"] ?? "";
            $decodedMessage = Html::entity_decode_deep(
    ...
    public function showForm($ID, $options = [])
    {
        ...
        echo '<td colspan="3"><input name="name" type="text" value="' .
            Html::cleanInputText($this->getField("name")) .
            '" style="width: 565px" /></td>';
        echo "</tr>";
        ...
        echo '<td colspan="3">';
        echo '<textarea name="message" rows="12" cols="80">' .
        ...
        if (Session::haveRight("is_displayed_onlogin", READ)) {
            ...
            echo "<td>";
            Dropdown::showYesNo(
                "is_displayed_onlogin",
                $this->fields["is_displayed_onlogin"],
                -1,
                [
                    "readonly" => !Session::haveRight(
                        "is_displayed_onlogin",
                        UPDATE,
                    ),
                ],
        ...
        if (Session::haveRight("is_displayed_onhelpdesk", READ)) {
            ...
            echo "<td>";
            Dropdown::showYesNo(
                "is_displayed_onhelpdesk",
                $this->fields["is_displayed_onhelpdesk"],
                -1,
                [
                    "readonly" => !Session::haveRight(
                        "is_displayed_onhelpdesk",
        ...
        if (Session::haveRight("is_displayed_oncentral", READ)) {
            ...
            Dropdown::showYesNo(
                ...
                $this->fields["is_displayed_oncentral"],
                -1,
                [
                    "readonly" => !Session::haveRight(
                        "is_displayed_oncentral",
    ...
    static function displayAlerts($params = [])
    {
        ...
        if ($alerts = self::findAllToNotify($p)) {
            foreach ($alerts as $alert) {
                ...
                echo "<div class='plugin_news_alert' data-id='" .
                    $alert["id"] .
                    "'>";
        ...
        if (
            ...
            ($alerts =
                self::findAllToNotify($hidden_params) &&
                !$p["show_hidden_alerts"])
        ) {
            echo "<div class='center'>";
            echo "<a href='" .
                Plugin::getWebDir("news") .
